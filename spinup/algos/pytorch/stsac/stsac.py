...
import spinup.algos.pytorch.stsac.core as core
...
def stsac(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
        steps_per_epoch=4000, epochs=100, replay_size=int(1e6), gamma=0.99, 
        polyak=0.995, pi_lr=1e-3, q_lr=1e-3, alpha=0.2, batch_size=100, start_steps=10000,
        update_after=1000, update_every=50, num_test_episodes=10, max_ep_len=1000, 
        logger_kwargs=dict(), save_freq=1, qleader=True, decay=True, reg=0, reg_decay=1e5, rollout_iters=1):
    """
    ...
    def prepare_params(data):
        ...
        if qleader:
            ...
            p1, p2 = list(itertools.chain(ac.q1.parameters(), ac.q2.parameters())), list(ac.pi.parameters())
            p1_size, p2_size = var_counts[1] + var_counts[2], var_counts[0]
        else:
        ...
        return f1, f2, p1, p2, p1_size, p2_size, q_info, pi_info

    def stac_update(data, reg):
        f1, f2, p1, p2, p1_size, p2_size, q_info, pi_info = prepare_params(data)
        ...
        # Record things
        logger.store(Loss1=f1.cpu().item(), Loss2=f2.cpu().item(), **q_info, **pi_info,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='stsac')
    ...
    stsac(lambda : gym.make(args.env), actor_critic=core.MLPActorCritic,
        ac_kwargs=dict(hidden_sizes=[args.hid]*args.l), 
        gamma=args.gamma, seed=args.seed, epochs=args.epochs,
        logger_kwargs=logger_kwargs, qleader=False)
